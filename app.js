...
app.get('/lapPenjualan', (req, res) => {
    if (req.session.role === 'admin' || req.session.role === 'kasir') { 
        ...
        console.log('Berhasil berada di halaman laporan stok barang');
        res.render('lapPenjualan', { title: 'Laporan Penjualan', penjualan: result, role: req.session.role });
    });
    } else {
        res.status(403).send('Forbidden');
    }
...
// app.post('/penjualan/bayar', (req, res) => {
//     const keranjang = req.session.keranjang || [];

//     // Hitung total harga dari semua item dalam keranjang
//     const totalHarga = keranjang.reduce((total, item) => total + item.total, 0);
...
//     if (isNaN(inputCash) || inputCash < totalHarga) {
//         console.error('Jumlah uang yang dibayarkan tidak valid. Input Cash:', inputCash, ', Total Harga:', totalHarga);
//         res.status(400).send('Jumlah uang yang dibayarkan tidak valid');
//         return;
//     }
  
//     // Hitung kembalian
//     const kembalian = inputCash - totalHarga;
...
app.post('/penjualan/save', (req, res) => {
    ...
    conn.query(insertQuery, [values], (err, result) => {
        ...
        // Hitung jumlah uang yang dibayarkan
        const inputCash = parseFloat(req.body.cash);
        ...
        // Setelah penyimpanan berhasil, buat kode transaksi baru
        req.session.currentTransactionID = generateUniqueTransactionID(req);
